...
def select_top_k_candidates(candidate_scores, k):
    ...
    # Flatten the candidate scores matrix to get all scores
    # candidate_scores shape: (num_beams, vocab_size)
    # Flattened shape: (num_beams * vocab_size)
    scores_flat = candidate_scores.flatten()
    
    # Get the top k scores and their flat indices
    top_k_scores, top_k_indices = torch.topk(scores_flat, k)
    
    # Convert flat indices back to beam_index and token_id
    # Beam index = flat_index // vocab_size
    # Token id = flat_index % vocab_size
    num_beams = candidate_scores.shape[0]
...
def append_tokens_to_beam_sequences(beam_sequences, beam_indices, token_ids):
    """Build the next set of beam sequences by gathering parent rows and appending new tokens."""
    # Gather parent beam rows according to beam_indices
    # beam_sequences shape: (num_beams, seq_len)
    # beam_indices shape: (k,)
    parent_sequences = beam_sequences[beam_indices]  # shape: (k, seq_len)
    
    # Add new token ids as the final column
    # token_ids shape: (k,)
    # Unsqueeze to shape (k, 1) to concatenate along the sequence dimension
    new_tokens = token_ids.unsqueeze(1)  # shape: (k, 1)
    
    # Concatenate along the sequence dimension (dim=1)
    new_sequences = torch.cat([parent_sequences, new_tokens], dim=1)  # shape: (k, seq_len + 1)
...
def select_best_finished_beam(finished_sequences, finished_scores, alpha):
    ...
    for seq, score in zip(finished_sequences, finished_scores):
        # Compute length penalty for this sequence
        seq_len = len(seq)
        penalty = compute_length_penalty(seq_len, alpha)
        
        # Normalize the score by the length penalty
        # Lower penalty (shorter sequences) gives higher normalized scores
